...
export function AlbumPage() {
  ...
  const [pos, setPos] = useState(1); // 001 = capa do álbum; 002..N = páginas de figurinhas
  ...
  // figurinhas da página de figurinhas atual (a capa é a página 001)
  ...
  const total = dados.album.paginas + 1; // 001 = capa + N páginas de figurinhas
    ...
    <div className="mx-auto max-w-4xl space-y-3">
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        ...
        <div className="flex min-h-[26rem] items-center justify-center p-4">
          {pos === 1 ? (
            // Página 001 = Foto da Capa (capa do álbum) — PDF §10
            dados.album.possuiCapa ? (
              <ImagemAuth src={colecaoApi.capaUrl} alt={`Capa de ${dados.album.nome}`} className="aspect-[3/4] max-h-[24rem] rounded-lg border border-border shadow-sm" />
            ) : (
              <div className="flex aspect-[3/4] max-h-[24rem] items-center justify-center rounded-lg border border-dashed border-border px-12 text-sm text-muted-foreground">
                Foto da Capa
              </div>
            )
            ...
            // Páginas 002+ = grade de figurinhas — PDF §11
        ...
        <div className="border-t border-border px-4 py-2.5">
          <Paginacao atual={pos} total={total} onMudar={setPos} />
        </div>
      </div>
