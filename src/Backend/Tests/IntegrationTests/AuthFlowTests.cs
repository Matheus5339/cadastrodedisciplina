...
{
    [Fact]
    public async Task Login_PorLogin_RetornaTokenEPerfil()
    {
        var client = factory.CreateClient();
        var resp = await client.PostAsJsonAsync("/api/auth/login", new { login = "autor", senha = ApiFactory.Senha });
        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);
        var auth = (await resp.Content.ReadFromJsonAsync<AuthResponseDto>(ApiFactory.Json))!;
        Assert.NotEmpty(auth.AccessToken);
        Assert.Equal("autor", auth.Usuario.Login);
    ...
    {
        var client = factory.CreateClient();
        var resp = await client.PostAsJsonAsync("/api/auth/login", new { login = "admin", senha = "errada123" });
    ...
    public async Task Refresh_E_Logout_ViaCookie()
    {
        ...
        var login = await client.PostAsJsonAsync("/api/auth/login", new { login = "colecionador", senha = ApiFactory.Senha });
